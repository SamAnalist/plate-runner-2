...
These are the working instructions for Claude on the Plate Runner project.
They mirror `GEMINI.md` (Gemini's instructions) and `AGENTS.md` (generic
agent overview) — all three should stay in sync. If this file ever
conflicts with `GEMINI.md` or `AGENTS.md`, treat that as a bug: reconcile
